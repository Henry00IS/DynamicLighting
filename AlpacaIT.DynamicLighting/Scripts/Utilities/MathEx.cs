...
namespace AlpacaIT.DynamicLighting
{
    ...
    internal static class MathEx
    {
        ...
        /// <summary>
        /// Gets the closest point on the bounding box. Identical to <see
        /// cref="Bounds.ClosestPoint"/> except that it's not a slow native call.
        /// </summary>
        /// <param name="point">The arbitrary point.</param>
        /// <returns>The point on the bounding box or inside the bounding box.</returns>
        public static unsafe Vector3 ClosestPointFast(this Bounds bounds, Vector3 point)
        {
            SVector4 center;
            SVector4 extent;
            SVector4 closest;
            *(Vector3*)&center = ((Vector3*)&bounds)[0];
            *(Vector3*)&extent = ((Vector3*)&bounds)[1];
            *(Vector3*)&closest = point;
            closest = SVector4.Max(center - extent, SVector4.Min(center + extent, closest));
            ...
            //var min = center;
            //var max = center;
            //UMath.Subtract(&min, &extents);
            //UMath.Add(&max, &extents);
            //
            //// clamp the point to the bounds of the box.
            //float x = Mathf.Max(min.x, Mathf.Min(max.x, point.x));
            //float y = Mathf.Max(min.y, Mathf.Min(max.y, point.y));
            //float z = Mathf.Max(min.z, Mathf.Min(max.z, point.z));
            //
            //Vector3 result; _ = &result; // bypass the constructor.
            //result.x = x;
            //result.y = y;
            //result.z = z;
            //return result;
