...
namespace AlpacaIT.DynamicLighting
{
    ...
    internal partial class DynamicLightingTracer
    {
        ...
        /// raycasts while background threads execute them.
        /// </summary>
        private unsafe class CallbackRaycastProcessor
        {
            /// <summary>
            /// The amount of commands that will be stored before scheduling it on the job system.
            /// </summary>
            ...
            /// <summary>Additional information per <see cref="RaycastCommand"/> for the results.</summary>
            private RaycastOriginMeta[] raycastCommandsOriginMeta = new RaycastOriginMeta[batchSize]; // 8 MiB

            /// <summary>Additional information per <see cref="RaycastCommand"/> for the results.</summary>
            private RaycastOriginMeta[] raycastCommandsOriginMetaAccumulator = new RaycastOriginMeta[batchSize]; // 8 MiB

            /// <summary>Additional information per <see cref="RaycastCommand"/> for the results.</summary>
            private IRaycastMissHandler[] raycastCommandsMissHandler = new IRaycastMissHandler[batchSize]; // 8 MiB (minimum)

            /// <summary>Additional information per <see cref="RaycastCommand"/> for the results.</summary>
            private IRaycastMissHandler[] raycastCommandsMissHandlerAccumulator = new IRaycastMissHandler[batchSize]; // 8 MiB (minimum)
            ...
            public void Add(RaycastCommand raycastCommand, RaycastOriginMeta raycastOriginMeta, IRaycastMissHandler raycastCommandMeta)
            {
                nativeRaycastCommandsAccumulator[countAccumulator] = raycastCommand;
                raycastCommandsOriginMetaAccumulator[countAccumulator] = raycastOriginMeta;
                raycastCommandsMissHandlerAccumulator[countAccumulator] = raycastCommandMeta;
                countAccumulator++;
            ...
            private void Execute()
            {
                ...
                if (countAccumulator > 0)
                {
                    ...
                    // copy the accumulators into the working arrays.
                    NativeArray<RaycastCommand>.Copy(nativeRaycastCommandsAccumulator, nativeRaycastCommands, countAccumulator);
                    Array.Copy(raycastCommandsMissHandlerAccumulator, raycastCommandsMissHandler, countAccumulator);
                    Array.Copy(raycastCommandsOriginMetaAccumulator, raycastCommandsOriginMeta, countAccumulator);
            ...
            private void ProcessResults()
            {
                for (int i = 0; i < count; i++)
                {
                    var origin = raycastCommandsOriginMeta[i];
                    var handler = raycastCommandsMissHandler[i];
                    var hit = nativeRaycastHits[i];
...
#endif
                    {
                        handler.OnRaycastMiss(origin.x, origin.y);
                    }
                    else
                    {
                        handler.OnRaycastHit(origin.x, origin.y);
                    }
                }
            }
            ...
            public void Dispose()
            {
                if (wasActive) throw new Exception("Unable to Dispose of an active " + nameof(CallbackRaycastProcessor));
