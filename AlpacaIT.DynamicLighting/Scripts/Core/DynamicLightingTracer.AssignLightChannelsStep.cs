...
namespace AlpacaIT.DynamicLighting
{
    ...
    {
        /// <summary>
        /// This step assigns light channels to all of the given <see cref="DynamicLight"/>
        /// instances in the scene. Every light must have a unique channel (indicating bit 0 to 31)
        /// to store their shadows in a single large uint bitmask. The algorithms ensures that
        /// overlapping lights do not get assigned the same channel, but the channel can be re-used
        /// when outside of the light's radius.
        /// </summary>
        public class AssignLightChannelsStep : IStep
        {
            private DynamicLight[] dynamicLights;

            /// <summary>Creates a new instance of <see cref="AssignLightChannelsStep"/>.</summary>
            /// <param name="dynamicLights">The light sources to be updated.</param>
            public AssignLightChannelsStep(DynamicLight[] dynamicLights)
            {
