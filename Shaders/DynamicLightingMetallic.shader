		...
		SubShader
		{
			...
			Pass
			{
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					float3 color = /*ambient +*/ Lo * lerp(1.0, ao, _OcclusionStrength); // _OcclusionStrength
					...
					for (uint k = 0; k < dynamic_lights_count; k++)
					{
						...
						float3 reflectDir = reflect(-light_direction, worldNormal);
						...
						light_final += light.color * attenuation * (diffusion + specular) * map;
					}
